...
cf = ax.contourf(
    X, Y, field, cmap="seismic", levels=np.linspace(-absmax / 5, absmax / 5, 20)
)
plt.colorbar(mappable=cf, ax=ax, shrink=0.75)
ax.coastlines()
...
cf = ax.contourf(X, Y, (U ** 2 + V ** 2) ** 0.5, cmap="viridis", levels=20)
plt.colorbar(mappable=cf, ax=ax, shrink=0.75)
skip = (slice(None, None, 5), slice(None, None, 5))
ax.quiver(X[skip], Y[skip], U[skip], V[skip], scale=40, width=0.001)
ax.coastlines()
