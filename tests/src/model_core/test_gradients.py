...
from tests.src.model_core.test_geography import (
    degrees_lon_to_meters,
    degrees_lat_to_meters,
)
...
def test_partial_x():
    ...
    p = {"x": 0, "y": 0, "z": 0, "t": 0}
    dx_m = degrees_lon_to_meters(np.array([0.5]), p["y"])[0]

    p["x"] = 0.1  # between index 0 and 1 of xfield.x
    V_xx_true = (2 - 0) / dx_m
    np.testing.assert_allclose(
        calculate_partials(p, xfield)[0], [V_xx_true, -V_xx_true, 2 * V_xx_true]
    )
    ...
    p["x"] = -0.1  # off bottom of array (but x is circular...)
    V_xx_true = (0 - (-1)) / dx_m
    np.testing.assert_allclose(
        calculate_partials(p, xfield, x_is_circular=True)[0],
        [V_xx_true, -V_xx_true, 2 * V_xx_true],
    )
...
def test_partial_y():
    ...
    np.testing.assert_allclose(
        calculate_partials(p, yfield)[1], [V_yx_true, -V_yx_true, 2 * V_yx_true]
    )
...
def test_partial_z():
    ...
    np.testing.assert_allclose(
        calculate_partials(p, zfield)[2], [V_zx_true, -V_zx_true, 2 * V_zx_true]
    )
...
def test_partial_t():
    tfield = {
        ...
        "U": np.array([0.5, 0.8, 10, 0.2, -0.5, 3]).reshape((6, 1, 1, 1)),
    }
    ...
    V_tx_true = (10 - 0.8) / 10
    np.testing.assert_allclose(
        calculate_partials(p, tfield)[3], [V_tx_true, -V_tx_true, 2 * V_tx_true]
    )

    p["t"] = 3444  # between index 3 and 4 of tfield.t
    V_tx_true = (-0.5 - 0.2) / 10
    np.testing.assert_allclose(
        calculate_partials(p, tfield)[3], [V_tx_true, -V_tx_true, 2 * V_tx_true]
    )
