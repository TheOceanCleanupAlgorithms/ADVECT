...
def single_diffusion_step(
    ...
    d_z = cl.Buffer(
        ...
        hostbuf=np.float64(z),
    )
    d_seed = cl.Buffer(
        CL_CONTEXT,
        cl.mem_flags.READ_WRITE | cl.mem_flags.COPY_HOST_PTR,
        hostbuf=np.uint32(seed),
    )
    d_hedz = cl.Buffer(
        CL_CONTEXT,
        cl.mem_flags.READ_WRITE | cl.mem_flags.COPY_HOST_PTR,
        hostbuf=np.float64(horiz_eddy_diff_z),
    )
    d_hedv = cl.Buffer(
        CL_CONTEXT,
        cl.mem_flags.READ_WRITE | cl.mem_flags.COPY_HOST_PTR,
        hostbuf=np.float64(horiz_eddy_diff_val),
    )
    d_vedz = cl.Buffer(
        CL_CONTEXT,
        cl.mem_flags.READ_WRITE | cl.mem_flags.COPY_HOST_PTR,
        hostbuf=np.float64(vert_eddy_diff_z),
    )
    d_vedv = cl.Buffer(
        CL_CONTEXT,
        cl.mem_flags.READ_WRITE | cl.mem_flags.COPY_HOST_PTR,
        hostbuf=np.float64(vert_eddy_diff_val),
    )

    d_out = cl.Buffer(
        CL_CONTEXT, cl.mem_flags.WRITE_ONLY | cl.mem_flags.COPY_HOST_PTR, hostbuf=out
    )
...
def test_diffusion(plot=False):
    ...
    dt = 1  # seconds
    seed = rng.integers(
        1, (1 << 32) - 1, len(p_depth)
    )  # for good random, must provide uniform seed in uint32 range
    ...
    drift = single_diffusion_step(
        p_depth, dt, seed, z_hd, horizontal_diffusivity, z_vd, vertical_diffusivity
    )
    ...
    bin_mean = np.array(
        [
            np.mean(
                drift[(p_depth > z - bin_radius) & (p_depth < z + bin_radius)], axis=0
            )
            for z in z_grid
        ]
    )
    ...
    true_std = np.array(
        [
            np.sqrt(2 * np.interp(z_grid, z_hd, horizontal_diffusivity) * dt),
            np.sqrt(2 * np.interp(z_grid, z_vd, vertical_diffusivity) * dt),
        ]
    )

    np.testing.assert_allclose(
        0, bin_mean[:, 0], atol=0.2 * np.mean(bin_std[:, 0])
    )  # mean within 1/5 O(std) of 0, p good
    np.testing.assert_allclose(0, bin_mean[:, 1], atol=0.2 * np.mean(bin_std[:, 1]))
    np.testing.assert_allclose(0, bin_mean[:, 2], atol=0.2 * np.mean(bin_std[:, 2]))
    np.testing.assert_allclose(
        true_std[0][1:], bin_std[:, 0][1:], rtol=0.1
    )  # bin at bottom edge is thrown off by boundary, ignore

    if plot:
        fig, ax = plt.subplots(
            2, 2, figsize=(9, 9), gridspec_kw={"width_ratios": [2, 1]}, sharey=True
        )
        for i, j in ((0, 0), (1, 2)):
            ax[i][0].plot(drift[:, j], p_depth, ".", markersize=0.5, label="particles")
            ax[i][0].plot(
                np.zeros_like(z_grid), z_grid, label="expected mean", linewidth=2
            )
