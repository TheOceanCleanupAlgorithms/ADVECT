...
def buoyancy_vertical_velocity(
    density: np.ndarray, radius: np.ndarray, corey_shape_factor: np.ndarray
) -> np.ndarray:
    """
    ...
    d_density = cl.Buffer(
        ...
        hostbuf=np.float64(density),
    )
    d_radius = cl.Buffer(
        CL_CONTEXT,
        cl.mem_flags.READ_ONLY | cl.mem_flags.COPY_HOST_PTR,
        hostbuf=np.float64(radius),
    )
    d_CSF = cl.Buffer(
        CL_CONTEXT,
        cl.mem_flags.READ_ONLY | cl.mem_flags.COPY_HOST_PTR,
        hostbuf=np.float64(corey_shape_factor),
    )
    ...
    prg.test_buoyancy_vertical_velocity(
        CL_QUEUE, density.shape, None, d_density, d_radius, d_CSF, d_out
    )
    CL_QUEUE.finish()
...
def test_kooi_2016(plot=False):
    ...
    rho = rng.uniform(
        rho_min, rho_max, nsamples
    )  # plastic density not stated; this mimics a selection of LDPE and HDPE

    # in figure 2, we see pictures of fragments.  they all look like wafers, but smaller ones are more rounded.
    corey_shape_factor_small = 0.5 / np.sqrt(
        1 * 1
    )  # figure 2a, looks like fragments are twice as long as they are thick
    corey_shape_factor_big = 1 / np.sqrt(
        4 * 4
    )  # figure 2b, looks like fragments are ~4x as long as they are thick
    ...
    size_big = np.linspace(
        1.5e-3, 5e-3, nsamples
    ...
    predicted_v_small = buoyancy_vertical_velocity(
        rho, r_small, np.full_like(rho, corey_shape_factor_small)
    )
    predicted_v_big = buoyancy_vertical_velocity(
        rho, r_big, np.full_like(rho, corey_shape_factor_big)
    )

    if plot:
        plt.figure(figsize=(8, 6))
        plt.plot(
            np.concatenate([size_small, size_big]) * 1e3,
            np.concatenate([predicted_v_small, predicted_v_big]),
            ".",
            label=f"predictions (rho ~ U({rho_min}, {rho_max}) kg m^-3)",
            color="tab:cyan",
            markersize=2,
        )
        plt.errorbar(
            np.mean(size_small) * 1e3 - 0.05,
            np.mean(predicted_v_small),
            np.std(predicted_v_small),
            capsize=3,
            color="tab:blue",
            zorder=3,
        )
        plt.errorbar(
            np.mean(size_big) * 1e3 - 0.05,
            np.mean(predicted_v_big),
            np.std(predicted_v_big),
            capsize=3,
            color="tab:blue",
        ...
        plt.errorbar(
            ...
            capsize=3,
            color="tab:green",
            label="observed distribution (Kooi 2016)",
            zorder=3,
        )
    ...
    assert (
        v_small_mean - 1.5 * v_small_std
        < np.mean(predicted_v_small)
        < v_small_mean + v_small_std * 1.5
    )
    assert (
        v_big_mean - 1.5 * v_big_std
        < np.mean(predicted_v_big)
        < v_big_mean + v_big_std * 1.5
    )
...
def compare_lebreton_2018(plot=False):
    ...
    boxplot_shapes = (
        ...
        * 1e-2
    )  # units: cm s^-1 (raw) converted to m s^-1
    ...
    p["dims"] = np.sort(
        rng.lognormal(
            mean=np.mean(np.log(size_classes / 2)), sigma=4, size=(nsamples, 3)
        ),
        axis=1,
    )
    ...
    for bnds in size_classes:
        rise_velocity_binned.append(
            p["rise_velocity"][
                (bnds[0] < p["size_class"]) & (p["size_class"] < bnds[1])
            ]
        )
