...
def test_calculate_windage_coeff():
    ...
    # at surface
    np.testing.assert_allclose(calculate_windage_coeff(r=1, z=0),
                               np.sqrt(density_ratio*drag_ratio*1))

    # partially submerged
    r, z = 1, -.2
    emerged_area = circular_segment_area(R=r, r=-z)
    np.testing.assert_allclose(calculate_windage_coeff(r=r, z=z),
                               np.sqrt(density_ratio * drag_ratio * emerged_area / (np.pi * r ** 2 - emerged_area)))
