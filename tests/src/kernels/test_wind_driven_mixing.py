...
def test_sample_concentration_profile(plot=False):
    ...
    measured_PDF, bin_edges = np.histogram(samples, bins=50, range=(MLD, 0), density=True)
    ...
    def test_and_plot():
        if plot:
            ...
            plt.hist(samples, bins=50, label='samples', range=(MLD, 0), orientation='horizontal', density=True)
            plt.ylim([MLD, 0])
            plt.legend()
            plt.ylabel('z')
...
def test_calculate_significant_wave_height():
    ...
    wind_stress = .75e-2**2 * water_density  # m/s  inverting kukulka's given frictional water velocity
    ...
    assert calculate_significant_wave_height(calculate_wind_stress(wind_speed_10m=40)) == 20
