    ...
    current = xr.Dataset(
        {
            ...
            "bathymetry": (["lat", "lon"], -1 * np.ones(U.shape)),  # gotta make sure particle is above bathymetry
        },
        coords={
            "lon": lon,
            "lat": lat,
...
if __name__ == '__main__':
    euler, taylor = compare_alg_drift_2d(initial_radius=.005, plot=True)
    euler, taylor = compare_alg_drift_3d(initial_radius=.005, plot=True)
