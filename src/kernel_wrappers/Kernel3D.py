...
class Kernel3D(Kernel):
    ...
    def __init__(
        ...
        self.current_bathy = current.bathymetry.values.astype(
            np.float32, copy=False
        ).ravel()
        # wind vector field
        ...
        seawater_density = forcing_data[Forcing.seawater_density].transpose(
            "time", "depth", "lat", "lon"
        )  # coerce values into correct shape before flattening
        ...
        self.seawater_density_t = seawater_density.time.values.astype(
            "datetime64[s]"
        ).astype(
            np.float64
        )  # float64 representation of unix timestamp
        self.seawater_density_values = seawater_density.rho.values.astype(
            np.float32, copy=False
        ).ravel()  # astype will still copy if variable is not already float32
        ...
        self.Z_out = np.full(
            (len(p0.depth) * len(self.out_time)), np.nan, dtype=np.float32
        )
        # physics
    ...
    def execute(self) -> xr.Dataset:
        ...
        (
            ...
            d_wind_U,
            d_wind_V,
            d_seawater_density_x,
            d_seawater_density_y,
            d_seawater_density_z,
            d_seawater_density_t,
            ...
            d_radius,
            d_p_density,
            d_corey_shape_factor,
            d_horizontal_eddy_diffusivity_z,
            d_horizontal_eddy_diffusivity,
            d_vertical_eddy_diffusivity_z,
            d_vertical_eddy_diffusivity,
        ) = (
            cl.Buffer(
                self.context,
            ...
            for hostbuf in (
                ...
                self.wind_U,
                self.wind_V,
                self.seawater_density_x,
                self.seawater_density_y,
                self.seawater_density_z,
                self.seawater_density_t,
        ...
        self.cl_kernel(
            ...
            d_wind_V,
            d_seawater_density_x,
            np.uint32(len(self.seawater_density_x)),
            d_seawater_density_y,
            np.uint32(len(self.seawater_density_y)),
            d_seawater_density_z,
            np.uint32(len(self.seawater_density_z)),
            d_seawater_density_t,
            np.uint32(len(self.seawater_density_t)),
            d_seawater_density_values,
            d_x0,
            d_y0,
            d_z0,
    ...
    def get_memory_footprint(self) -> dict:
        ...
        seawater_density_bytes = (
            self.seawater_density_x.nbytes
            + self.seawater_density_y.nbytes
            + self.seawater_density_z.nbytes
            + self.seawater_density_t.nbytes
