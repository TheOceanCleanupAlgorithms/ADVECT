...
class Kernel2D(Kernel):
    ...
    def __init__(
        ...
        if "wind" in forcing_data:
            ...
            self.windage_coefficient = np.nan  # to flag the kernel that windage is disabled
    ...
    def execute(self) -> xr.Dataset:
        ...
        d_current_x, d_current_y, d_current_t,\
            ...
            (cl.Buffer(self.context, cl.mem_flags.READ_ONLY | cl.mem_flags.COPY_HOST_PTR, hostbuf=hostbuf)
             for hostbuf in
             (self.current_x, self.current_y, self.current_t,
        ...
        self.cl_kernel(
            ...
            d_wind_U, d_wind_V,
            d_x0, d_y0, d_release_date,
            np.uint32(self.advection_scheme), np.float64(self.windage_coefficient), np.float64(self.eddy_diffusivity),
