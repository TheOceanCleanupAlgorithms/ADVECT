...
def openCL_advect(
    ...
    current = current.sel(
        time=slice(advect_time[0], advect_time[-1])
    )  # trim vector fields to necessary time range
    if wind is not None:
    ...
    p0_chunk = p0.copy()
    chunk_paths = []
    for advect_time_chunk, out_time_chunk, current_chunk, wind_chunk in zip(
        advect_time_chunks, out_time_chunks, current_chunks, wind_chunks
    ):
        print(
            f"Chunk {len(chunk_paths)+1:3}/{len(current_chunks)}: "
            f"{current_chunk.time.values[0]} to {current_chunk.time.values[-1]}..."
        )

        num_timesteps_chunk = (
            len(advect_time_chunk) - 1
        )  # because initial position is given!
        out_timesteps_chunk = len(out_time_chunk) - 1  #

        # create the kernel wrapper object, pass it arguments
        with ProgressBar():
            print(
                f"  Loading currents and wind..."
            )  # these get implicitly loaded when .values is called on current_chunk variables
            kernel = create_kernel(
                ...
                num_particles=num_particles,
                dt=dt,
                start_time=advect_time_chunk[0],
                num_timesteps=num_timesteps_chunk,
                save_every=save_every,
        ...
        P_chunk = create_dataset_from_kernel(
            kernel=kernel,
        ...
        p0_chunk.loc[
            p0_chunk.release_date > advect_time_chunk[-1], ["lat", "lon"]
        ] = p0[["lat", "lon"]]
...
def create_kernel(
    ...
    return Kernel2D(
        ...
        current_t=current.time.values.astype("datetime64[s]").astype(
            np.float64
        ),  # float64 representation of unix timestamp
        current_U=current.U.values.astype(
            np.float32, copy=False
        ).ravel(),  # astype will still copy if field.U is not already float32
        ...
        wind_t=wind.time.values.astype("datetime64[s]").astype(
            np.float64
        ),  # float64 representation of unix timestamp
        wind_U=wind.U.values.astype(
            np.float32, copy=False
        ).ravel(),  # astype will still copy if field.U is not already float32
        ...
        save_every=save_every,
        X_out=np.full(
            (num_particles * out_timesteps), np.nan, dtype=np.float32
...
def create_dataset_from_kernel(
    kernel: Kernel2D, release_date: np.ndarray, advect_time: pd.DatetimeIndex
) -> xr.Dataset:
    ...
    P = xr.Dataset(
        ...
        coords={
            ...
            "time": advect_time[1:],
        },  # initial positions are not returned
    )
    return P
