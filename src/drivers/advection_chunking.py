...
def chunk_advection_params(
    ...
    List[pd.DatetimeIndex], List[pd.DatetimeIndex], List[xr.Dataset], List[xr.Dataset]
]:
    ...
    if available_bytes_for_field <= 0:
        raise RuntimeError(
            "Particles take up all of device memory; no space for field chunks. "
            "Decrease number of particles and try again."
        )
    num_chunks = math.ceil(
        field_bytes / available_bytes_for_field
    ...
    while True:
        ...
        if all(
            ...
            < device_bytes
            for current_chunk, wind_chunk, out_time_chunk in zip(
                current_chunks, wind_chunks, out_time_chunks
            )
        ):
            break
        num_chunks += 1
...
def estimate_memory_bytes(
    ...
    current_bytes = 2 * 4 * np.prod(current.U.shape) + 8 * (  # two 32-bit fields
        len(current.lon) + len(current.lat) + len(current.time)
    )  # the 3 64-bit coordinate arrays
    if wind is not None:
        wind_bytes = 2 * 4 * np.prod(wind.U.shape) + 8 * (  # two 32-bit fields
            len(wind.lon) + len(wind.lat) + len(wind.time)
        )  # the 3 64-bit coordinate arrays
    else:
        wind_bytes = 0
    output_bytes = (
        2 * 4 * num_particles * out_timesteps
    )  # two 32-bit variables for each particle for each timestep
