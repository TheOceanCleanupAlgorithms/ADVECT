...
@click.command()
@click.option(
    "-o", "out_path", required=True, type=click.Path(exists=False, dir_okay=False)
)
...
def generate_sourcefile(out_path: str, num_particles: int):
    ...
    p0 = pd.DataFrame(
        data=[
            ocean_points[i]
            for i in np.random.randint(0, len(ocean_points), size=num_particles)
        ],
        columns=["lon", "lat"],
    )
    p0["release_date"] = np.concatenate(
        (
            np.full(num_particles // 2, np.datetime64("2015-01-01T12")),
            np.full(
                num_particles // 2 + num_particles % 2, np.datetime64("2015-06-01")
            ),
        )
    )
