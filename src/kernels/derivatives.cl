...
vector field_derivative_meters(particle p, field2d field, double dt, double derivative_coeff) {
    ...
    //////////// Return lagrangian derivative of the field
    vector derivative_meters;
    derivative_meters.x = ut + ux * uv.x + uy * uv.y;
    derivative_meters.y = vt + vx * uv.x + vy * uv.y;

    derivative_meters.x = derivative_coeff * derivative_meters.x;
    derivative_meters.y = derivative_coeff * derivative_meters.y;

    return derivative_meters;
}
...
vector particle_acceleration_meters(vector previous_speed, vector second_previous_speed, double dt, double acceleration_coeff) {
    vector acceleration_meters = {.x = (previous_speed.x - second_previous_speed.x) / dt, 
                                 .y = (previous_speed.y - second_previous_speed.y) / dt};

    acceleration_meters.x *= acceleration_coeff;
    acceleration_meters.y *= acceleration_coeff;

    return acceleration_meters;
}