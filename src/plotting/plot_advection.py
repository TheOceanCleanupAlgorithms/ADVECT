...
def plot_ocean_advection(
    outputfile_path: str, lon_range=(-180, 180), lat_range=(-90, 90)
):
    P = xr.open_dataset(outputfile_path)
    # plot le advection
    proj = ccrs.PlateCarree()
    ...
    (dot,) = ax.plot(
        P.isel(time=0).lon, P.isel(time=0).lat, ".", color="tab:blue"
...
@click.command()
@click.argument(
    "outputfile_path", type=click.Path(exists=True, dir_okay=False, readable=True)
)
