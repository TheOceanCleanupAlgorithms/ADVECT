...
diverge_v = np.array([[0, 1, 0],
                      [0, 0, 0],
                      [0, -1, 0]])
diverge_u = np.array([[0, 0, 0],
                      [-1, 0, 1],
                      [0, 0, 0]])

div = np.array([convolve(u, diverge_u, mode='constant') for u in U.values]) + \
      np.array([convolve(v, diverge_v, mode='constant') for v in V.values])
...
plt.contourf(smooth, levels=np.linspace(-.01, .01, 50), cmap=plt.get_cmap('seismic'))
plt.colorbar()
